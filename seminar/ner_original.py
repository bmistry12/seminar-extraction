...
def checkForName(name):
    ...
    if (checkFile(name, "res/names.male") or checkFile(name, "res/names.female") or checkFile(name, "res/names.family")):
        #word is in file of names
        isName = True
    elif (name in title) :
        #the word is a title - indicates a name will be coming
        isName = True
    elif (checkFile(name, speakFile)):
        #the word is a name in the data extracted from the original training data
        isName = True
    elif (wikify(name)) :
